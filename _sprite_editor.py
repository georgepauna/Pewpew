...
# Per-BMP sha256 manifest the RG auto-updater fetches first to figure
# out which BMPs actually changed before downloading any (see
# BMP_MANIFEST_PATH in pewpew.py). Regenerated by save() so the file
# always matches whatever BMPs the editor just wrote to disk.
BMP_HASH_MANIFEST_PATH = ART / "bmp_manifest.json"
...
class Editor:
    ...
    def save(self):
        ...
        # Hash manifest for the RG auto-updater. Walk every BMP under
        # art/sprites so any new sprite the user adds via the editor
        # is automatically in the next push. Sorted-key dump keeps the
        # JSON diff-friendly.
        bmps = {}
        for bmp in sorted(SPRITES.glob("*.bmp")):
            try:
                bmps[f"art/sprites/{bmp.name}"] = hashlib.sha256(
                    bmp.read_bytes()).hexdigest()
            except Exception:
                pass
        BMP_HASH_MANIFEST_PATH.write_text(json.dumps(bmps, indent=2))
        self.dirty = False
        self.flash_t = 1800
        self.flash_msg = (f"saved manifest + {written} PNGs + "
                          f"{len(engine_out)} engine entries + "
                          f"{len(bmps)} BMP hashes")
